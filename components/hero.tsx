...
function DesktopDeviceMockup({
  ...
  return (
    ...
    >
      ...
      >
        <div className="h-96 w-96 rounded-full bg-primary/15 blur-[120px]" />
      </motion.div>

      <motion.div className="relative z-10" animate={{ y: [0, -10, 0] }} transition={{ duration: 4.6, repeat: Infinity, ease: "easeInOut" }}>
        <div className="relative mx-auto h-[520px] w-[260px] overflow-hidden rounded-[2.5rem] border border-border/60 bg-card shadow-2xl shadow-primary/10">
          <div className="absolute inset-0 rounded-[2.5rem] ring-1 ring-inset ring-foreground/5" />
          ...
          <div className="flex flex-col items-center px-5 pb-4 pt-4">
            <div className="mb-3 flex h-20 w-20 items-center justify-center rounded-full bg-primary/15 ring-2 ring-primary/20">
            ...
            <div className="mt-3 rounded-xl border border-primary/20 bg-primary/5 px-3 py-1.5 text-center">
          ...
          <div className="flex flex-col gap-2 px-4">
            {[
              ...
              <div key={card.title} className="flex items-center gap-2.5 rounded-xl bg-secondary/60 p-3 transition-colors">
      ...
      <motion.div
        ...
        className="glass-card absolute left-[-60px] top-1/8 z-20 rounded-xl p-3 shadow-lg"
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] font-semibold text-primary">+$50.00</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="glass-card absolute -right-4 bottom-1/3 z-20 rounded-xl p-3 shadow-lg"
      >
        <div className="flex items-center gap-2">
...
function MobileAnimatedContainer() {
  ...
  return (
    ...
    >
      <motion.div
        className="pointer-events-none absolute -inset-4 rounded-[30px] border border-primary/20"
        animate={{ rotate: 360 }}
        transition={{ duration: 18, repeat: Infinity, ease: "linear" }}
      />
      <motion.div
        className="pointer-events-none absolute -inset-7 rounded-[36px] border border-dashed border-primary/10"
        animate={{ rotate: -360 }}
        transition={{ duration: 26, repeat: Infinity, ease: "linear" }}
      />
      <motion.div
        className="pointer-events-none absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/15 blur-[100px]"
        animate={{ scale: [0.92, 1.05, 0.92], opacity: [0.5, 0.75, 0.5] }}
        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      />

      <motion.div
        className="glass-card relative overflow-hidden rounded-[30px] p-6 shadow-2xl shadow-primary/10"
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {["Chat", "Booking", "Products", "Events"].map((item) => (
            <div
              key={item}
              className="rounded-xl border border-border/40 bg-card/80 px-3 py-2 text-center text-xs font-medium text-foreground transition-colors"
            >
              {item}
            </div>
          ))}
      ...
      <motion.div
        className="glass-card absolute -left-1 top-6 rounded-xl px-3 py-2 shadow-lg sm:-left-8 sm:-top-6"
        animate={{ y: [0, -10, 0] }}
      ...
      <motion.div
        className="glass-card absolute -right-1 bottom-8 rounded-xl px-3 py-2 shadow-lg sm:-right-2 sm:bottom-10"
        animate={{ y: [0, 11, 0] }}
        transition={{ duration: 2.8, repeat: Infinity, ease: "easeInOut" }}
      >
        <div className="text-[11px] font-semibold text-primary">+12 orders</div>
      </motion.div>
    </motion.div>
  )
}
...
export function Hero() {
  ...
  const glow = useMotionTemplate`radial-gradient(480px circle at ${pointerX}% ${pointerY}%, rgba(16,185,129,0.18), transparent 58%)`
  ...
  return (
    ...
    >
      ...
      />
      {/* Radial glow gradient top */}
      <div className="pointer-events-none absolute -top-40 left-1/2 h-[600px] w-[900px] -translate-x-1/2 rounded-full bg-primary/[0.06] blur-[120px]" />
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col items-center gap-8 px-4 py-10 sm:gap-10 sm:px-6 sm:py-14 md:py-16 lg:flex-row lg:gap-14 lg:px-8 lg:py-20">
        ...
        <div className="flex w-full max-w-2xl flex-1 flex-col items-center text-center lg:items-start lg:text-left">
          <motion.div
            ...
            animate="visible"
            custom={0}
            className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-4 py-1.5 sm:mb-6"
          >
            <div className="h-1.5 w-1.5 animate-pulse rounded-full bg-primary" />
            <span className="text-[11px] font-medium tracking-wide text-primary sm:text-xs">Creator Economy Platform</span>
          </motion.div>
          ...
          >
            <motion.div whileHover={{ y: -2, scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Link
                href="/signup"
                className="group flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:shadow-xl hover:shadow-primary/25 hover:brightness-110 sm:w-auto animate-pulse-glow"
              >
                SIGN UP FREE
            ...
            <motion.div whileHover={{ y: -2, scale: 1.01 }} whileTap={{ scale: 0.985 }}>
              <Link
                href="#demo"
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-border/60 bg-card/80 px-7 py-3.5 text-sm font-medium text-foreground backdrop-blur-sm transition-colors hover:border-primary/20 hover:bg-secondary sm:w-auto"
              >
          ...
          >
            {features.map((feature) => (
              <motion.div
                ...
                className="flex items-center gap-1.5 rounded-lg border border-border/40 bg-card/60 px-2.5 py-1.5 backdrop-blur-sm sm:gap-2 sm:px-3 sm:py-2"
              >
          ...
          >
            {[
              { icon: Users, stat: "10k+", label: "active creators" },
              { icon: TrendingUp, stat: "3.1x", label: "avg growth" },
              { icon: ShieldCheck, stat: "Secure", label: "payments" },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-2 rounded-xl border border-border/40 bg-card/60 px-3 py-2.5 backdrop-blur-sm">
                <item.icon className="h-4 w-4 text-primary" />
                <p className="text-xs text-muted-foreground">
                  <span className="font-semibold text-foreground">{item.stat}</span> {item.label}
                </p>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div
            ...
            animate="visible"
            custom={7}
            className="mt-5 w-full max-w-xl overflow-hidden rounded-xl border border-primary/15 bg-primary/5"
          >
            <motion.div
        ...
        <div className="relative flex w-full flex-1 flex-col items-center justify-center gap-6 pt-1 sm:gap-8 sm:pt-3 lg:flex-row lg:gap-8 lg:pt-0">
          ...
          <motion.div
            className="glass-card absolute -right-1 top-4 hidden rounded-xl px-3 py-2 shadow-lg lg:block"
            animate={{ y: [0, -6, 0] }}
          ...
          <motion.div
            className="glass-card absolute bottom-4 left-1 hidden rounded-xl px-3 py-2 shadow-lg lg:block"
            animate={{ y: [0, 7, 0] }}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-background to-transparent sm:h-40" />
    </section>
  )
}
